...
write.csv(tab2_descriptive_export,'tab_onlyhealthmodule.csv')
...
model_2 <- df_regressions_bothm %$%
  lm(HOOPs_COOPs ~ urban + hhsize + factor(survey))

# Include hhead info
model_3 <- df_regressions_bothm %$% 
  lm(HOOPs_COOPs ~ urban + hhsize +hhead_female +hhead_married +hhead_age + factor(survey))

# Include hospitalization
model_4 <- df_regressions_bothm %$% 
  lm(HOOPs_COOPs ~ urban + hhsize +hhead_female +hhead_married +hhead_age + episodic_hosp + factor(survey))
...
Same but for final sample HOSP==1
...
tab_model( model_2, model_3, model_4,
          ...
          p.style = "asterisk",
          title = "Regression Models: OOPs in health module (DV), and OOPs in consumption module",
          terms = c("(Intercept)","Consumption","urbanTRUE","hhsize","hhead_female","hhead_married","hhead_age") ,
          dv.labels = c("With household characteristics" , "With household head characteristics","With hospitalization"),
          pred.labels = c("Intecept","OOPs consumption module","Urban","Household size","Female head","Married head","Age head") )
```
...
tab_model(model_22, model_23, model_24,
          robust = TRUE,
          p.style = "asterisk",
          ...
          dv.labels = c("With household characteristics" , "With household head characteristics", "With hosp"),
...
```{r regression-styles, echo=FALSE, message=FALSE, eval=FALSE}
...
df_reg_health <- df_nontidy %>%
  ...
  left_join(table_descriptive2_wide)



# Model a - only recall and length
model_a <- df_reg_health %$% 
  lm(`Mean OOPs Health Module` ~ + `Avg. Recall Health Module`)

# Model b - include the styles
model_b <- df_reg_health %$% 
  lm(`Mean OOPs Health Module` ~ + `Avg. Recall Health Module`+ n_episodic + n_utilization + n_direct)
...
tab_model(model_a, model_b,
          robust = TRUE,
          p.style = "asterisk",
          title = "Regression Models: Effect of style on mean of surveys")

```

```{r style_micro , echo=FALSE, warning=FALSE, message=FALSE, eval=FALSE }
...
df_styles <- df_styles %>% 
  mutate_at(c("n_episodic","n_direct","n_utilization"),funs(replace_na(.,0))) %>% 
  mutate(Health_nonhealth = Health_nonhealth*100)

# Run model - effect of styles: only styles
model_71 <- df_styles %$%  
  lm(Health_nonhealth ~ n_episodic + n_utilization + n_direct)


# Model 2 include n-items and recall period
model_72 <- df_styles %$%  
  lm(Health_nonhealth ~ n_episodic + n_utilization + n_direct + `Avg. Recall Health Module`)


# Model 3 include household characteristics
model_73 <- df_styles %$%  
  lm(Health_nonhealth ~  n_episodic + n_utilization + n_direct + `Avg. Recall Health Module` + hhsize +  urban + hhead_female + hhead_married + hhead_age)

# Model 4 include hospitalization
model_74 <- df_styles %$%  
  lm(Health_nonhealth ~ n_episodic + n_utilization + n_direct + `Avg. Recall Health Module` + hhsize +  urban + hhead_female + hhead_married + hhead_age + episodic_hosp)
