...
# Add data on number of food and non-food questions
input_n_food_nonfood <- read.csv('input_numbernonfood.csv') %>% 
  rename(n_food = `number.of.food.questions`,
         n_nonfood = `number.of.non.food.questions`) %>% 
  filter(n_food!='.') %>% 
  mutate(n_food = n_food %>% as.double(),
         n_nonfood = n_nonfood %>% as.double())


df_reg_health <- df_reg_health %>% 
  inner_join(input_n_food_nonfood)
...
# Model c - include the styles and number of questions
model_c <- df_reg_health %$% 
  lm(`Percent OOPs Health Module` ~  `Avg. Recall Health Module`+ n_episodic + n_utilization + n_direct + n_food + n_nonfood)


tab_model(model_a, model_b, model_c,
          robust = TRUE,
          p.style = "asterisk",
...
df_styles <- df_styles %>% 
  left_join(input_n_food_nonfood)
...
# Model 3 number of food and non-food consumption questions
model_73 <- df_styles %$%  
  lm(HOOPS_Nonhealth ~ n_episodic + n_utilization + n_direct + `Avg. Recall Health Module` + n_food + n_nonfood)

# Model 4 include household characteristics
model_74 <- df_styles %$%  
  lm(HOOPS_Nonhealth ~  n_episodic + n_utilization + n_direct + `Avg. Recall Health Module` + n_food + n_nonfood + hhsize +  urban + hhead_female + hhead_married + hhead_age)

# Model 5 include hospitalization
model_75 <- df_styles %$%  
  lm(HOOPS_Nonhealth ~ n_episodic + n_utilization + n_direct + `Avg. Recall Health Module` + n_food + n_nonfood + hhsize +  urban + hhead_female + hhead_married + hhead_age + episodic_hosp)

tab_model(model_71, model_72, model_73, model_74,model_75,
          robust = TRUE,
          p.style = "asterisk",
