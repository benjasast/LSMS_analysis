...
```{r descriptive_table_bothmodules, echo=FALSE, warning=FALSE, message=FALSE}
...
table_descriptive2_wide <- long_descriptive2 %>% 
  pivot_wider(names_from = Classification, values_from = percent, names_prefix = "Percent_") %>% 
  mutate_at(c("Percent_episodic","Percent_hosp","Percent_direct","Percent_utilization","Percent_trad"),funs(replace_na(.,0))) %>% 
  mutate_at(c("Percent_episodic","Percent_hosp","Percent_direct","Percent_utilization","Percent_trad"),funs(round(.,1)))
...
tab_model(model_1, model_2, model_3, model_4,
          ...
          dv.labels = c("Simple Model", "With household characteristics" , "With household head characteristics","With hospitalization"),
...
# Simple model (OOPs and nitems and recall)
model_21 <- df_reg2 %>% 
  filter(survey!="Bulgaria_2007") %$%
...
tab_model(model_21, model_22, model_23, model_24,
          robust = TRUE,
          p.style = "asterisk",
          ...
          dv.labels = c("Simple Model", "With household characteristics" , "With household head characteristics", "With hosp"),
...
```{r regressions_OOPs_Positive, echo=FALSE, warning=FALSE, message=FALSE}
...
```{r regression_noFE_Positive, echo=FALSE, warning=FALSE, message=FALSE}
...
# Models for styles
```{r models_style, echo=FALSE, warning=FALSE, message=FALSE, eval=FALSE }
...
df_styles %>% 
  select(survey,Percent_direct,Percent_episodic,Percent_hosp,Percent_trad,Percent_utilization) %>% 
  distinct()


# Run model - effect of styles: only styles
model_71 <- df_styles %$%  
  lm(Health ~ Percent_episodic + Percent_utilization + Percent_direct + Percent_hosp + Percent_trad)


# Model 2 include n-items and recall period
model_72 <- df_styles %$%  
  lm(Health ~ Percent_episodic + Percent_utilization + Percent_direct + Percent_hosp + Percent_trad + `Items Health Module` + `Avg. Recall Health Module`)


# Model 3 include household characteristics
model_73 <- df_styles %$%  
  lm(Health ~ Percent_episodic + Percent_utilization + Percent_direct + Percent_hosp + Percent_trad + `Items Health Module` + `Avg. Recall Health Module` + hhsize +  urban + hhead_female + hhead_married + hhead_age)

# Model 4 include hospitalization
model_74 <- df_styles %$%  
  lm(Health ~ Percent_episodic + Percent_utilization + Percent_direct + Percent_hosp + Percent_trad + `Items Health Module` + `Avg. Recall Health Module` + hhsize +  urban + hhead_female + hhead_married + hhead_age + episodic_hosp)
