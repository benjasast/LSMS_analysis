...
obs <- df_nontidy %>% 
  select(hhid_compilation, healthm_oops, health_consumption, survey, nonhealth_consumption) %>% 
  filter(healthm_oops>-1,health_consumption>-1) %>%  # take NAs out 
  group_by(survey) %>% 
  summarise(Observations = n(),
            `Mean Non-Health Consumption` = round( mean(nonhealth_consumption, na.rm = TRUE),1),
            `Mean OOPs Health Module` = round( mean(healthm_oops, na.rm = TRUE),1),
            `Mean OOPs Consumption Module`  = round( mean(health_consumption, na.rm = TRUE),1)) %>% 
  rename(Survey = survey)
...
tab_health_over_consumption <- tab_mean_oops %>% 
  ...
  ungroup() %>% 
  select(survey, mhealthc_mhealthm, healthc_healthm, healthm_healthc, trim_mhealthc_mhealthm) %>% 
  distinct() %>% 
...
tab_health_over_consumption_zero %>% 
  ...
  ylab("Share") +
  geom_hline(yintercept=1, alpha=3, linetype = "dashed") +
  scale_y_continuous(breaks = seq(0,19,1), limits = c(0,19))
```
