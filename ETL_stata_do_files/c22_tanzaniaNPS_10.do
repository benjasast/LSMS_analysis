...
cd "~/Dropbox/LSMS_Compilation/Data/tanzania_NPS_08_10_12_14/NPS_10/"
...
use "TZY2.HH.Consumption.dta" , clear
...
keep hhid y2_hhid hhsize hhweight strata urban region area mainland intmonth intyear ///
	quarter *consumption che*

order hhid y2_hhid *consumption
...
duplicates drop y2_hhid, force // ok
...
* Max level educ
merge 1:1 y2_hhid indidy2 using HH_SEC_C , keepus(hh_c03 hh_c07)
...
* Reshape to hh unit
reshape wide q* , i(y2_hhid) j(shock_category)
...
rename q06_1 	death_ageyears
rename q06_2 	death_agemonths
...
use aux1, clear
	...
	merge 1:1 y2_hhid using aux4
		drop if _merge!=3 // ~700 obs no shock data
		drop _merge
...
drop hhid
rename y2_hhid hhid		
drop if mi(food_consumption) // they are all missing.
		
		
* Save
cd "~/Dropbox/LSMS_Compilation/Analysis/Output_Files/"
gen survey = "Tanzania_2011"
save tanzaniaNPS_2010 , replace
